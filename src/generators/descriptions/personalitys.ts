...
export default function personalitys() {
    ...
    if (type === 1) {
        ...
        name = `${sample(nm11)} ${sample(nm23)} ${sample(
            nm24
        ...
        name2 = `But focus on ${sample(nm25)} as this is what ${sample(nm25)} ${sample(
            nm14
        )}. ${sample(nm15)} because of this and ${sample(nm25)} ${
            sample(nm22)[1]
        }, ${sample(nm16)}.`;
        name3 = `Fair is fair though, ${sample(nm23)} ${sample(nm17)}. ${sample(
            nm25
        )} ${sample(nm21)[0]} and ${sample(nm21)[0]} ${
            nm18[rnd8]
        }, ${sample(nm19)}.`;
        name4 = `Unfortunately ${sample(nm25)} ${sample(nm22)[1]} ${
            nm20[rnd10]
        }.`;
    } else {
        ...
        name = `${sample(nm1)} ${sample(nm23)} ${sample(nm24)}, but ${
            nm2[rnd2]
        } ${sample(nm25)} ${sample(nm21)[0]} and ${
            sample(nm21)[0]
        }. Of course ${sample(nm25)} also ${sample(nm21)[0]}, ${
            nm21[rnd14][0]
        } and ${sample(nm21)[0]}, but ${sample(
            nm3
        )} being ${sample(nm22)[0]} as well.`;
        name2 = `${sample(nm25)} ${sample(nm21)[1]} though, this is what ${
            nm25[0]
        } ${sample(nm4)}. ${sample(nm5)} count on this and ${
            nm25[1]
        } ${sample(nm21)[1]} ${sample(nm6)}.`;
        name3 = `Nobody's perfect of course and ${sample(nm23)} has ${
            nm7[rnd7]
        } too. ${sample(nm25)} ${sample(nm22)[1]} and ${
            nm22[rnd19][1]
        } ${sample(nm8)}${sample(nm9)}.`;
        name4 = `Fortunately ${sample(nm25)} ${sample(nm21)[1]} ${
            nm10[rnd10]
        }.`;
    }
    let result = '';
